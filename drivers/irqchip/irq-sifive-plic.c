...
#define	NANHU_DEBUG_HWIRQ_A		42
#define	NANHU_DEBUG_HWIRQ_B		44

#define PLIC_QUIRK_EDGE_INTERRUPT	0

static bool nanhu_debug_hwirq(irq_hw_number_t hwirq)
{
	return hwirq == NANHU_DEBUG_HWIRQ_A || hwirq == NANHU_DEBUG_HWIRQ_B;
}
...
static void plic_irq_eoi(struct irq_data *d)
{
	...
	if (nanhu_debug_hwirq(d->hwirq))
		pr_info("[ree-plic] eoi cpu=%u hwirq=%lu\n",
			raw_smp_processor_id(), d->hwirq);
...
static void plic_handle_irq(struct irq_desc *desc)
{
	...
	while ((hwirq = readl(claim))) {
		if (nanhu_debug_hwirq(hwirq))
			pr_info("[ree-plic] claim cpu=%u hwirq=%lu\n",
				raw_smp_processor_id(), hwirq);
